...
func (tp *PrintAndGo) sendJob(line string) {
	tp.Lock()
	tp.stats.numSent++
	tp.stats.lastCmd = line
	tp.Unlock()
	//command := strings.Split(line, ";") // remove Gcode comments if any
	tp.log.Println("serialOut Job Gcode (discard):", line)
	// _, _ = io.Discard.Write([]byte(line + "\n"))
	_ = line
	//_, err = tp.serialOut.Write(append([]byte(line), '\r', '\n'))
	// if err != nil {
	// 	// tp.log.Printf("Error writing to serial port: %v", err)
	// }
	// return err
