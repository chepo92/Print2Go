...
func (wapi *WebApi) enqueuePrint(instr store.Stream, shutdown bool) error {
	...
	fmt.Printf("[ENQUEUE] Print requested\n")
	...
	// 2. Enviar archivo G-code línea por línea
	fmt.Printf("[ENQUEUE] Sending G-code file to serial\n")
	if err := wapi.serial.SendGcodeFile(instr); err != nil {
		instr.Close()
		return fmt.Errorf("print failed: %w", err)
	}

	instr.Close()
	fmt.Printf("[ENQUEUE] Print finished\n")

	// 3. Shutdown (lo dejamos simple por ahora)
	if shutdown {
		fmt.Printf("[ENQUEUE] Shutdown requested (not implemented yet)\n")
	}
