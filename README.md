...
```shell
$ GOOS=linux GOARCH=amd64 go build -o ./builds/PrintAndGoLinux . # Example for Linux 
$ CGO_ENABLED=0 GOARCH=arm64 go build -o ./builds/PrintAndGoRPI3 . # Example for a raspberry Pi 3
$ GOOS=linux GOARCH=mipsle GOMIPS=softfloat go build -o ./builds/PrintAndGoMIPSle . # Example for MIPS Little Endian
```
...
In the container, remove existing app files: 
`cd /app/ && rm -r *`
Then in the host, copy to container: 
`docker cp ./ go-builder:/app/`
Then you can build and copy back to host 
