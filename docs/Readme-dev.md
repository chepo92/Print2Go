...
1. Compile 
```shell
$ go build -o ./dist/Print2Go.exe .   # in case you are running in and building for Windows
$ go build -o ./dist/Print2Go .       # in case you are running in and building for Linux
```

If you want to cross compile:

```shell
$ GOOS=linux GOARCH=amd64 go build -o ./dist/Print2Go-Linux . # Example for Linux 
$ CGO_ENABLED=0 GOARCH=arm64 go build -o ./dist/Print2Go-ARM64_RPI3 . # Example for a raspberry Pi 3
$ GOOS=linux GOARCH=mipsle GOMIPS=softfloat go build -o ./dist/Print2Go-MIPSle . # Example for MIPS Little Endian
$ GOOS=windows GOARCH=amd64  go build -o ./dist/Print2Go-Win64.exe . # Example for Windows 64 bit
$ GOOS=windows GOARCH=386  go build -o ./dist/Print2Go-Win32.exe . # Example for Windows 32 bit
```
...
`docker build -f docker/Dockerfile --target builder -t go-builder-dev .`

2. After first time build, can be run
`docker run --rm -it -v ${PWD}:/src -w /src/src go-builder-dev sh`

2.1 And Create container 
`docker run -it --name go-builder-dev go-builder:1.0 sh`

3. After first run, can be re-started, with interactive terminal
`docker start -a -i go-builder-dev`

4. In the interactive terminal (IT), build the Print2Go project (use a the cross compile command from above if the case): 
`go build -o ./dist/Print2Go .`

5. Once build, copy files back to host (eg. after cross compile, get the binaries), run in the host: 
Copy one file
`docker cp go-builder:/app/dist/Print2GoLinux ./dist/Print2GoLinux`
Copy all folder
`docker cp go-builder:/app/dist/ .`
...
`docker cp ./src/ go-builder-dev:/app/`
...
`GOOS=linux GOARCH=mipsle GOMIPS=softfloat go build -o ./dist/Print2Go_mipsle ./src`
